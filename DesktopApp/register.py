...
class RegisterPage(tb.Frame):
    ...
    def register(self):
        ...
        if not username or not password:
            tb.Messagebox.show_error("Fields cannot be empty", "Error")
            return

        if register_user(username, password):
            tb.Messagebox.show_info("Account created successfully!", "Success")
            self.master.show_screen("LoginPage")
        else:
            tb.Messagebox.show_error("Username already exists", "Registration Failed")
